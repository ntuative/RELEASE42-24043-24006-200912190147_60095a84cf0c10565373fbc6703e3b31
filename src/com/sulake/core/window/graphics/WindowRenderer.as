package com.sulake.core.window.graphics
{
   ...
   public class WindowRenderer implements IWindowRenderer
   {
      ...
      private var var_611:ISkinContainer;
      
      private var var_1449:Rectangle;
      ...
      public function render() : void
      {
         ...
         while(_loc4_ < _loc2_)
         {
            _loc1_ = var_543.pop() as WindowController;
            if(!_loc1_.disposed)
            {
               _loc3_ = getWindowRendererItem(_loc1_);
               var_294.x = _loc3_.dirty.x;
               var_294.y = _loc3_.dirty.y;
               var_294.width = _loc3_.dirty.width;
               var_294.height = _loc3_.dirty.height;
               renderWindowBranch(_loc1_,_loc3_,var_294);
            }
            _loc4_++;
         }
      }
      ...
      {
         param1.removeEventListener(WindowDisposeEvent.const_718,windowDisposedCallback);
         var _loc2_:WindowRendererItem = var_293[param1] as WindowRendererItem;
         if(_loc2_ != null)
         {
            _loc2_.dispose();
      ...
      private function renderWindowBranch(param1:WindowController, param2:WindowRendererItem, param3:Rectangle) : void
      {
         ...
         if(param1.visible)
         {
            if(getDrawLocationAndClipRegion(param1,param3,var_1450,var_1449))
            {
               if(param2.render(param1,var_1450,var_1449))
               {
      ...
      public function registerRenderable(param1:IWindow) : void
      {
         ...
         if(_loc2_ == null)
         {
            _loc2_ = new WindowRendererItem(this,_drawBufferAllocator,var_611);
            var_293[param1] = _loc2_;
            param1.addEventListener(WindowDisposeEvent.const_718,windowDisposedCallback);
         }
      }
      ...
      public function addToRenderQueue(param1:IWindow, param2:Rectangle, param3:uint) : void
      {
         ...
         if(getWindowRendererItem(param1).invalidate(param1,param2,param3))
         {
            ...
            var_294.x = param2.x - param1.x;
            var_294.y = param2.y - param1.y;
            var_294.width = param2.width;
            var_294.height = param2.height;
            if(param1.testParamFlag(WindowParam.const_32))
            {
               while(param1.testParamFlag(WindowParam.const_32))
               {
                  ...
                  var_294.offset(param1.x,param1.y);
                  param1 = _loc4_;
               }
            }
            getWindowRendererItem(param1).invalidate(param1,var_294,WindowRedrawFlag.const_222);
            if(var_543.indexOf(param1) == -1)
            {
               var_543.push(param1);
            }
         }
