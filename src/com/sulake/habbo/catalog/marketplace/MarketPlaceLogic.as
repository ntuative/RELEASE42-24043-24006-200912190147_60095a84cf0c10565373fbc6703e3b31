package com.sulake.habbo.catalog.marketplace
{
   ...
   public class MarketPlaceLogic implements IMarketPlace
   {
      ...
      public const const_1605:int = 3;
      
      public const const_1500:int = 2;
      ...
      {
         var _loc2_:XmlAsset = var_69.assets.getAssetByName(param1) as XmlAsset;
         if(_loc2_ == null)
         {
            return null;
         }
      ...
      {
         var_69.getOwnMarketPlaceOffers();
      }
      ...
      private function onConfirmDialogEvent(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         switch(param2.name)
         {
            ...
            case "confirm_buy_button":
               var_69.buyMarketPlaceOffer(var_449);
               if(var_133)
               {
                  var_133.dispose();
               }
      ...
      public function onBuyResult(param1:IMessageEvent) : void
      {
         ...
         if(parser.result == 1)
         {
            if(parser.offerId > 0)
            {
               newItem = var_226.getValue(parser.requestedOfferId) as MarketPlaceOfferData;
               var_226.remove(parser.requestedOfferId);
               newItem.offerId = parser.offerId;
               newItem.price = parser.newPrice;
               var_226.add(parser.offerId,newItem);
            }
            else
            {
               var_226.remove(parser.requestedOfferId);
         ...
         {
            updateItem = var_226.getValue(parser.requestedOfferId) as MarketPlaceOfferData;
            if(updateItem)
            {
               updateItem.offerId = parser.offerId;
               updateItem.price = parser.newPrice;
               var_226.add(parser.offerId,updateItem);
            }
            var_226.remove(parser.requestedOfferId);
            var_449 = parser.offerId;
            showConfirmation(const_1500);
            _visualization.listUpdatedNotify();
         }
      ...
      {
         var_69.getPublicMarketPlaceOffers(param1,-1);
      }
      ...
      public function onOwnOffers(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            var_640.add(_loc4_.offerId,_loc5_);
         }
      ...
      public function redeemSoldOffers() : void
      {
         var _loc2_:int = 0;
         var _loc3_:* = null;
         var _loc1_:Array = var_640.getKeys();
         for each(_loc2_ in _loc1_)
         {
            _loc3_ = var_640.getValue(_loc2_);
            if(_loc3_.status == MarketPlaceOfferState.const_354)
            {
               var_640.remove(_loc2_);
            }
         }
         var_69.redeemSoldMarketPlaceOffers();
         _visualization.listUpdatedNotify();
      }
      ...
      private function showConfirmation(param1:int) : void
      {
         ...
         if(var_133)
         {
            ...
            var_133.center();
            _loc3_ = var_226.getValue(var_449) as MarketPlaceOfferData;
            if(!_loc3_)
            {
               return;
            }
      ...
      {
         var_69.getPublicMarketPlaceOffers("",param1);
      }
      ...
      public function onOffers(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc3_.offers)
         {
            ...
            var_226.add(_loc4_.offerId,_loc6_);
         }
         if(_visualization == null)
         {
            return;
      ...
      {
         var_69.redeemExpiredMarketPlaceOffer(param1);
      }
      
      public function buyOffer(param1:int) : void
      {
         var_449 = param1;
         showConfirmation(const_1533);
      }
